...
### Markdown — two paths
- **Batch (Reports → Run Markdown):** items 60+ days on floor; applies tier %, stores `original_price`, sets `markdown_applied = true` and `needs_new_tag = true`. Floor Sheet prints for physical re-pricing.
...
- ⚠️ Scanner sets `needs_new_tag = false`, so scanner markdowns do NOT appear in Bulk Tag Reprint (which keys off `true`). By design (red-sharpie assumption).
...
### Database Health & Bulk Tag Reprint (Admin → Data Management, manager+ only)
...
- **Bulk Tag Reprint:** lists items where `needs_new_tag = true`; filter by markdown-applied vs manually-flagged + consignor + category. Print Selected / Print All via `printTags()` (ZPL/QZ Tray, max 40/batch, dry-run honored). Post-print "Mark as printed?" → sets `tag_printed = true`, `needs_new_tag = false`.
...
**Markdown Scanner reporting:** Scanning is pure DB input (audit_log is the system of record). The on-screen session list (`marked` state / `localStorage` `markdown.scanner.session`) is a cosmetic per-device scratchpad only. "Print Report" queries `getMarkdownAuditReport(date)` in db.js — filters `audit_log` by `action='markdown'` + date, joins `items` (active only), resolves names from `user_roles` + `details.markdown_applied_by_name`. Groups report by user; date picker + All Stations / My Scans Only filter. Reports from any machine reflect all stations.
...
| 2026-05-21 | Decoupled Markdown Scanner reporting from per-device localStorage. Print Report now queries audit_log (getMarkdownAuditReport in db.js): date picker, All Stations / My Scans Only filter, groups by user, active items only, prices from audit log details. Added details.markdown_applied_by_name for durable attribution. On-screen session list kept as cosmetic scratchpad; New Session still clears it. 171 tests pass. CDN purge: finished. |
