...
- **QR mobile upload now unreachable UI-wise:** removing the QR buttons orphaned `PhotoUpload.jsx` + `/upload/:token` route + `createUploadSession()` (no caller generates tokens anymore). Left in place, not deleted — cleanup candidate.
