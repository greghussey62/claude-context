...
- **Drive import consolidated into the single item-photo pipeline (2026-07-14) — needs a Vercel deploy to take effect.** Drive import now writes straight to `items.photos` (was the isolated `shopify_photos` lane); `+ Add Photo` / QR removed from Item Detail + EditItemModal, replaced by one `⬇ Import Photos` button; cap raised 4→10. **Verify in prod after deploy:** Drive round-trip (photograph → folder → Import → appears in item strip). `shopify_photos` table now dormant/orphaned (old `shopify/{item_id}/` storage files too) — drop when ready (see schema.sql). **Google-key gotcha still applies:** store `GOOGLE_SERVICE_ACCOUNT_KEY` as **base64** in Vercel (`loadPrivateKey()` decodes base64/PEM/escaped-\n). Next block: push `items.photos` to Shopify product images (#33/#34).
- **QR mobile upload REMOVED (2026-07-14):** deleted `PhotoUpload.jsx`, the `/upload/:token` route, `createUploadSession()`, and the DB side (`photo_upload_sessions` table, `upload_item_photos` RPC, and the anon `anon_photo_update` grant on `items`). **⚑ Re-run `supabase/schema.sql`** (or run the three `drop` statements in the ITEM PHOTO SYSTEM section) to actually remove them from the live DB and close the old anon photo-write path.
