...
- **Drive import consolidated into the single item-photo pipeline — DONE, deployed & verified in prod (2026-07-14).** Drive import writes straight to `items.photos` (was the isolated `shopify_photos` lane); `+ Add Photo` / QR removed from Item Detail + EditItemModal, replaced by one `⬇ Import Photos` button; cap raised 4→10; imported Drive originals moved to `processed/{item_id}/`. Full round-trip confirmed live (photograph → folder → Import → shows on item; originals nested per-item). `shopify_photos` table now dormant/orphaned (old `shopify/{item_id}/` storage files too) — drop when ready (see schema.sql). **Google-key gotcha still applies:** store `GOOGLE_SERVICE_ACCOUNT_KEY` as **base64** in Vercel (`loadPrivateKey()` decodes base64/PEM/escaped-\n). Next block: push `items.photos` to Shopify product images (#33/#34).
- **QR mobile upload REMOVED (2026-07-14):** deleted `PhotoUpload.jsx`, the `/upload/:token` route, `createUploadSession()`, and the DB side (`photo_upload_sessions` table, `upload_item_photos` RPC, and the anon `anon_photo_update` grant on `items`). `supabase/schema.sql` **re-run in prod** — objects dropped, old anon photo-write path closed.
...
### Photo Management
Max **10** photos/item (`MAX_PHOTOS` in `photos.js`, was 4). Path `items/{contractNum}/{itemId}/photo_{slot}.jpg`. Compressed server-side (`sharp`, max 1200px, JPEG 82%) during the Drive import. **Single add-photo path (2026-07-14): the Google Drive import.** `⬇ Import Photos` on Item Detail + EditItemModal → `importDrivePhotos()` → `api/drive-intake.js` pulls the Drive inbox (up to remaining slots), appends public URLs to `items.photos`, returns the updated array. Realtime on `items` UPDATE refreshes the strip live. Delete = ✕/Remove (drops the URL, purges storage on save). Purge old photos via Admin → Data Management (sold/returned/pulled items; deletes Storage files + sets `photos = []`, preserves record). *(Retired 2026-07-14: direct file-picker `+ Add Photo`, the separate `shopify_photos` approve/reject lane, and the entire QR phone-upload system — `PhotoUpload.jsx`, `/upload/:token`, `createUploadSession`, `photo_upload_sessions`, and the `upload_item_photos` RPC — all deleted.)*
...
- **Shopify product sync (#33)** + **AI Shopify descriptions (#34):** not built. (Drive photo intake — the upstream piece — is DONE; see Fully Built + Session Log 2026-07-13. Natural next step: push `approved` shopify_photos to Shopify product images.)
...
**Supabase:** Auth (email/password). All queries via `@supabase/supabase-js` in `src/lib/db.js`. RLS on every table — authenticated get full access via `auth_all`; anon SELECT on `pos_sessions`/`appointment_settings`, anon INSERT on `intake_appointments`. Storage bucket `item-photos` (public). Realtime in EditItemModal/ItemDetail/display. RPCs: `next_receipt_number()`, `complete_pos_sale(...)` (SECDEF, atomic sale), `count_slot_bookings(date, slot)` (SECDEF, anon).
...
- `shopify_photos` — **DEPRECATED 2026-07-14, dormant.** Was the isolated Drive-intake staging lane; the import now writes straight to `items.photos`, so nothing reads or writes this table. Kept (not dropped) so old rows survive; `drop table if exists shopify_photos;` when you no longer need them.
...
**Routing (App.jsx):** all routes under one `<Layout>`. Public (no auth): `/display`, `/book`, `/reset-password`. Auth but outside force-password-change: `/change-password`, `/profile`. Everything else through `CheckPasswordChange` (redirects to `/change-password` if `force_password_change`). No lazy loading.
...
**Vercel API routes:** `api/send-receipt.js` (Resend, `RESEND_API_KEY`) · `api/admin-users.js` (Supabase Admin API, `SUPABASE_SERVICE_ROLE_KEY`) · `api/auth-events.js` (public lockout tracking) · `api/qz-sign.js` (QZ Tray request signing, `QZ_PRIVATE_KEY`) · `api/drive-intake.js` (Google Drive photo intake — `GOOGLE_SERVICE_ACCOUNT_EMAIL`/`GOOGLE_SERVICE_ACCOUNT_KEY`/`DRIVE_INTAKE_FOLDER_ID`, `SUPABASE_SERVICE_ROLE_KEY`; validates caller JWT, compresses with `sharp`, **appends public URLs to `items.photos`** at `items/{contract}/{item}/photo_{slot}.jpg` up to the 10-photo cap, moves Drive originals to a `processed/` subfolder). `send-receipt`/`admin-users`/`drive-intake` validate caller JWT; server-side keys never reach browser.
...
| 2026-07-13 (2) | Shopify module **phases 2–3 + key fix — now live & verified in prod.** Phase 2 UI (commit `9fc13a9`): "Shopify Photos" section on `ItemDetail.jsx` — "Import photos from Drive" button → `importDrivePhotos()` (new `src/lib/shopifyPhotos.js`, posts to `/api/drive-intake` with caller JWT), result banner (imported N / no-new / per-file failures), thumbnail grid via `getShopifyPhotos()` (new in db.js, returns rows + public URLs). Phase 3 review (commit `13286bb`): per-thumbnail ✓ Approve / ✕ Reject → `setShopifyPhotoStatus(id,status)` (status + `reviewed_at`), optimistic update + revert-on-error, status badge + colored border; added `.btn-success` + `.badge-approved/rejected/imported` to global.css. Single-row, no bulk, no undo. All isolated from `items.photos`/QR flow. **Key-loading fix (commit `e35a96a`):** prod import first failed with `ERR_OSSL_UNSUPPORTED` — Vercel mangled the raw-PEM `GOOGLE_SERVICE_ACCOUNT_KEY` newlines. `loadPrivateKey()` now accepts base64-encoded PEM (reliable on Vercel), raw/escaped-`\n` PEM, and strips stray quotes; **key re-stored as base64** in Vercel. End-to-end import + approve/reject confirmed working in prod. Build clean, 171 tests pass. CDN purge: finished. |
| 2026-07-14 | **Photo systems consolidated to one pipeline (Drive import → `items.photos`).** Rationale: same photos serve both the in-store display and Shopify, so the isolated `shopify_photos` lane + manual add paths were redundant. `api/drive-intake.js` rewritten to append imported photos straight to `items.photos` (path `items/{contract}/{item}/photo_{slot}.jpg`, slot continues past highest `photo_N`, capped at cap, returns updated array). `MAX_PHOTOS` 4→10 (`photos.js`, covers ItemDetail/EditItemModal/CameraMode/PhotoUpload). Item Detail + EditItemModal: removed `+ Add Photo`/QR/QR-modal + the Shopify approve-reject section, single `⬇ Import Photos` button, import result banner + Realtime refresh. Removed dead `getShopifyPhotos`/`setShopifyPhotoStatus` from db.js; `shopify_photos` table marked deprecated in schema.sql (kept, not dropped). Orphaned but left in place: `PhotoUpload.jsx`/`/upload/:token`/`createUploadSession` (no token generator remains). One targeted `eslint-disable react-hooks/immutability` on ItemDetail's `useEffect(()=>{load()})`. Build clean, 171 tests pass. **⚑ Needs Vercel deploy; verify Drive round-trip in prod. Old `shopify/{item_id}/` storage files + `shopify_photos` rows now orphaned.** CDN purge: finished. |
| 2026-07-14 (2) | **Removed the now-orphaned QR mobile-upload system** (dead after the photo consolidation). Deleted `src/pages/PhotoUpload.jsx`, the `/upload/:token` route + import in `App.jsx`, and `createUploadSession()` in `photos.js`. schema.sql: replaced the QR blocks with idempotent `drop`s — `drop policy anon_photo_update on items`, `drop table photo_upload_sessions cascade`, `drop function upload_item_photos` — which also closes the old **anon UPDATE grant on `items`** (a small security tightening: no anon photo-write path remains). Build clean, 171 tests pass. **⚑ Re-run `supabase/schema.sql` to drop the objects in the live DB.** CDN purge: finished. |
| 2026-07-14 (3) | **Shipped + verified in prod.** Drive intake `moveToProcessed` now nests originals under `processed/{item_id}/` (generalized `ensureProcessedFolder`→`ensureSubfolder(parent,name)`, two-level resolve; old flat `processed/` files untouched — commit `06710f2`). User re-ran `schema.sql` (QR objects dropped), deployed to prod via `vercel --prod`, and confirmed the **full Drive round-trip live**: Import → photos compressed to `items/{contract}/{item_id}/`, appended to `items.photos`, shown on item, originals in `processed/{item_id}/`. (Debug note: a "photos not showing" report was an item mix-up — upload had gone to item `2-03`, not `2-01`; `item_id` drives both the Storage path and the Drive folder name, so they always agree. No bug.) CDN purge: finished. |
