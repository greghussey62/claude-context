...
- **Tag price formatting:** change `toFixed(2)` → `Math.round(Number(item.price || 0))` in `tagPrinter.js` `renderMerchandiseTag` (line ~102) — removes cents, e.g. `$85.00` → `$85`. Shown in Labelary, not yet applied.
