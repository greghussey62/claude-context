...
| 2026-05-20 | Admin tabs reorganized into five labeled groups (Catalog, Hardware, Store Rules, People, Maintenance), Catalog leftmost (most-used). Each group is a vertical block with a small uppercase label above its row of tab buttons; groups laid out horizontally with `flexWrap`. Role gating unchanged — Users + Permissions still admin-only; Maintenance (Data Management) still manager+. If a group's tab list is empty after role filtering (e.g. People for a non-admin manager), the whole block including the label is hidden via `.filter(g => g.tabs.length > 0)`. Tab state (`tab` / `setTab`) and content-switching `{tab === '...' && <Comp />}` blocks untouched. CDN purge: finished. |
